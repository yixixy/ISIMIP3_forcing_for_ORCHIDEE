...
ISIMIP_old_dir = '/speng/workdir/xiyi/DATA/ISIMIP_for_ORCHIDEE/';
ISIMIP_new_dir = '/speng/workdir/xiyi/DATA/ISIMIP_for_ORCHIDEE_reso2/';

% ISIMIP_old_dir = 'D:\forcing\ISIMIP/';
% ISIMIP_new_dir = 'D:\forcing\ISIMIP_reso1/';

scens = {'obsclim'; 'historical'; 'ssp126'; 'ssp370'; 'ssp585'};
yrs = [1901 2016; 1901 2014; 2015 2100; 2015 2100; 2015 2100];
yrs_ORCHIDEE = [1901 2016; 1901 2014; 2015 2100; 2015 2100; 2015 2100];
...
%maskr = imread('/home/xiyi/DATA/Tiff/World/continent_reso2.tif');
%maskr = double(maskr);
%maskr(maskr==255) = nan;
%maskr(~isnan(maskr)) = 1;
...
load('/workdir/xiyi/DATA/ISIMIP_for_ORCHIDEE/land_mask_2deg_CRUJRA.mat', 'land_mask_CRUJRA');
maskr = land_mask_CRUJRA';

Areas = area_weighted_any([-90 90], [-180 180], reso)*1e6; % m2
Areas(isnan(land_mask_CRUJRA)) = 1.000000020040877e+20;
Areas = Areas';
% figure; imagesc(Areas)
...
for isc = 3%1:length(scens)
    gcms_isc = gcms{isc, 1};
    for igcm = 1%1:length(gcms_isc)
        ...
        for iyr = 2071%yrs_ORCHIDEE(isc, 1):yrs_ORCHIDEE(isc, 2)
            disp(iyr)
            tic
            filename_new = [ISIMIP_new_subdir, gcms_isc{igcm}, '_twodeg_daily_', num2str(iyr), '.nc'];
            ...
            ncwriteatt(filename_new, '/', 'create date', '2024-04-08');
